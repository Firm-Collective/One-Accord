...
import {
  CretePostSchema,
  CreteContentPostSchema,
  ParseCretePostSchema,
  type CretePostSchemaType,
  CreteContentPostSchemaType,
} from '../schemas';
...
type CreatePostSchemaTypePick = Pick<
  CretePostSchemaType,
  | 'user_id'
  | 'activity_id'
  | 'category_id'
...
const useLiveFeed = ({ refetch }: Props) => {
  ...
  const onValid = async (data: CreteContentPostSchemaType) => {
    ...
    if (
      ...
      !postData.sentiment_id ||
      !postData.keywords_id ||
      !postData.event_id ||
      !postData.media_type_id
    ) {
